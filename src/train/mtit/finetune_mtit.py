...
"""finetune_mtit.py: script to fine-tune IT or MTIT model"""

"""credit: source code isnpired from from https://github.com/leehanchung/lora-instruct/"""
...
train_data_json_path = "tefcop_tr_cont_aux5_2.json" # replace with the training data structured in alpaca instruction format for a setting
lora_adapter_output_path = "./lora-alpaca"
...
class TrainConfig:
    ...
    lora_target_modules: List[str] = field(
        default_factory=lambda:['q_proj','v_proj']# ["query_key_value"]
    )
...
def train(
    ...
    data_path: str = train_data_json_path,
    output_dir: str = lora_adapter_output_path,
    # training hyperparams
    batch_size: int = 128,
    micro_batch_size: int = 4,
    num_epochs: int = 15,
    learning_rate: float = 3e-4,
    cutoff_len: int = 256,#256
    val_set_size: int = 1,#2000,
    # lora hyperparams
    lora_r: int = 8,
    lora_alpha: int = 16,
    lora_dropout: float = 0.05,
    lora_target_modules: List[str] = ['q_proj','v_proj'],#["query_key_value", "xxx"],
    ...
    device_map =  {'': 0}#"auto" #changed ashutosh
    ...
    #torch.save(model.state_dict(),"tefcop_lora_models/lora_tr_base.pth")
